...
import { advanceChaseYaw, advanceHeading, raceSpeed } from '../src/handling.ts';

const angle = (a, b) => Math.abs(Math.atan2(Math.sin(a - b), Math.cos(a - b)));

function run(hz, drifting, speed = raceSpeed(31), direction = 1) {
  const dt = 1 / hz;
  ...
  for (let frame = 0; frame < hz; frame++) {
    ...
    state = advanceHeading(state, { steer: steering ? direction : 0, speed, handling: 1,
      drifting: drifting && steering, boostHandling: 1, wobble: 0, dt });
    ...
    if (steering) turnAtRelease = Math.abs(state.moveYaw);
  }
  return { ...state, turnAtRelease, maxSlip, maxCameraAngle };
}

const normal = run(60, false);
const drift = run(60, true);
const drift30 = run(30, true);
const boosted = run(60, true, raceSpeed(53));
