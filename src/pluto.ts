...
const COLLAPSE_DURATION = 8;
const BREAK_LENGTH = 27;
...
export class PlutoUltimate {
  ...
  private spawnMark(racers: PlutoRacer[]) {
    ...
    const ahead = clamp(target.speed * (STRIKE_WARNING + 0.12), 38, 94);
    ...
    const danger = new THREE.Mesh(fracturedRoad(laneWidth - 0.65, BREAK_LENGTH), new THREE.MeshBasicMaterial({ color: 0x080610, transparent: true, opacity: 0, depthWrite: false, side: THREE.DoubleSide, polygonOffset: true, polygonOffsetFactor: -2 }));
    danger.rotation.x = -Math.PI / 2;
    danger.position.y = 0.03;
    const outline = new THREE.Mesh(fracturedRoad(laneWidth + 0.55, BREAK_LENGTH + 1.4), new THREE.MeshBasicMaterial({ color: 0xffc46e, transparent: true, opacity: 0.52, depthWrite: false, side: THREE.DoubleSide, polygonOffset: true, polygonOffsetFactor: -3 }));
    ...
    root.add(paw);
    const tongueMaterial = new THREE.MeshStandardMaterial({ color: 0xf67491, roughness: 0.38, emissive: 0x6c1639, emissiveIntensity: 0.28 });
    const tongueShaft = new THREE.Mesh(new THREE.CylinderGeometry(0.75, 1.15, 1, 14), tongueMaterial);
    const tongueTip = new THREE.Mesh(new THREE.SphereGeometry(1.25, 18, 12), new THREE.MeshStandardMaterial({ color: 0xffa2ad, roughness: 0.42, emissive: 0x93294f, emissiveIntensity: 0.18 }));
    tongueShaft.visible = tongueTip.visible = false;
    this.scene.add(tongueShaft, tongueTip);
    ...
    for (let i = 0; i < 12; i++) {
      ...
      const radius = i % 3 === 0 ? 7.2 : 5.1;
      const end = new THREE.Vector3(Math.cos(angle) * Math.min(laneWidth * 0.46, radius), 0.085, Math.sin(angle) * Math.min(BREAK_LENGTH * 0.48, radius * 2.2));
    ...
    for (let i = 0; i < 28; i++) {
      ...
      const z = (Math.cos(i * 3.21) * 0.45) * BREAK_LENGTH;
    ...
    this.scene.add(root);
    this.marks.push({ route: point.route, progress, lane, halfWidth: laneWidth / 2, halfLength: BREAK_LENGTH / 2, position, tangent: point.tangent.clone(), right: point.right.clone(), age: 0, hit: new Set(), root, danger, outline, paw, tongueShaft, tongueTip, debris, cracks, fired: false });
  }
  ...
  update(dt: number, racers: PlutoRacer[]): PlutoImpact[] {
    ...
    if (caster && viewer) {
      const striking = [...this.marks].reverse().find((mark) => mark.age < STRIKE_WARNING + 1.2);
      const anchor = striking ? this.track.routeAt(striking.route, striking.progress) : this.track.at(wrap(viewer.progress + 78 / this.track.length));
      ...
      const lookAt = striking?.position ?? viewer.position;
      this.group.rotation.y = Math.atan2(lookAt.x - this.group.position.x, lookAt.z - this.group.position.z);
      ...
      if (this.giant) {
        this.giant.position.y = Math.sin(this.age * 2.2) * 0.25 + (this.marks.some((mark) => mark.age > STRIKE_WARNING - 0.24 && mark.age < STRIKE_WARNING + 0.3) ? -2.4 : 0);
    ...
    for (let i = this.marks.length - 1; i >= 0; i--) {
      ...
      if (!mark.fired && mark.age >= STRIKE_WARNING) {
        ...
        (mark.danger.material as THREE.MeshBasicMaterial).opacity = 1;
        (mark.outline.material as THREE.MeshBasicMaterial).color.setHex(0xf58851);
        impacts.push({ position: mark.position.clone() });
      }
      const tongueAge = mark.age - (STRIKE_WARNING - 0.56);
      const extending = clamp(tongueAge / 0.56, 0, 1);
      const extension = mark.age < STRIKE_WARNING ? extending : 1 - clamp((mark.age - STRIKE_WARNING) / 0.62, 0, 1);
      mark.tongueShaft.visible = mark.tongueTip.visible = extension > 0.01;
      if (extension > 0.01) {
        const facing = new THREE.Vector3(Math.sin(this.group.rotation.y), 0, Math.cos(this.group.rotation.y));
        const mouth = this.group.position.clone().addScaledVector(facing, 4.5).add(new THREE.Vector3(0, 16.2, 0));
        const tip = mouth.clone().lerp(mark.position.clone().add(new THREE.Vector3(0, 0.3, 0)), extension);
        const span = tip.clone().sub(mouth);
        mark.tongueShaft.position.copy(mouth).addScaledVector(span, 0.5);
        mark.tongueShaft.scale.set(1, Math.max(0.01, span.length()), 1);
        mark.tongueShaft.quaternion.setFromUnitVectors(new THREE.Vector3(0, 1, 0), span.normalize());
        mark.tongueTip.position.copy(tip);
      }
      ...
      if (mark.fired && mark.age > STRIKE_WARNING + COLLAPSE_DURATION) {
        this.scene.remove(mark.root, mark.tongueShaft, mark.tongueTip);
        mark.tongueShaft.geometry.dispose(); (mark.tongueShaft.material as THREE.Material).dispose();
        mark.tongueTip.geometry.dispose(); (mark.tongueTip.material as THREE.Material).dispose();
...
function plutoMouth(racer: TongueRacer) {
  ...
  return racer.position.clone().addScaledVector(right, 0.68).addScaledVector(forward, -0.84).add(new THREE.Vector3(0, 2.48, 0));
}
